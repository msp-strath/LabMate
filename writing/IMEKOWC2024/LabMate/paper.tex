...
builtin left division operator \texttt{\textbackslash} to compute a
least squares fit. Let us step through how we intend to use LabMate to
help us deliver this solution.

We then ask for the type of the resulting solution. Note
that for reasons of space only twelve values are listed. A real
measurement would contain considerably more values:
\begin{verbatim}
%> z3 :: [ 3 x 1 ] double
z3 = [ 0; 0; 0 ]
%> i3 :: [ 3 x 1 ] double
i3 = [ 1; 1; 1 ]
%> times :: [ 12 x 1 ] double
% define times as a column vector
