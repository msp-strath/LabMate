...
mutual
 ...
 _^t_ : {sc sc' : Nat} -> Term sc -> sc <= sc' -> Term sc'
 ne n ^t th = ne (n ^n th)
 atom a ^t th = atom a
 ...
 _^tz_ : {sc sc' : Nat} -> Bwd (Term sc) -> sc <= sc' -> Bwd (Term sc')
 [] ^tz th = []
...
mutual
 ...
 data Type (sc : Nat) : Set where
  pi : (a : Type sc) -> (b : {sc' : Nat} -> (th : sc <= sc') -> El th a -> Type sc') -> Type sc
  sg : (a : Type sc) -> (b : {sc' : Nat} -> (th : sc <= sc') -> El th a -> Type sc') -> Type sc
...
mutual
 ...
 quoteType : {sc : Nat} -> Type sc -> Term sc
 ...
 unquoteEl : {sc sc' : Nat} -> (a : Type sc) -> (th : sc <= sc') -> Neutral sc' -> El th a
 ...
 quoteEl : {sc sc' : Nat} -> (a : Type sc) -> (th : sc <= sc') -> El th a -> Term sc'
 ...
 quoteList : {sc sc' : Nat} -> (a : Type sc) -> (th : sc <= sc') -> List (Neutral sc' + El th a) -> Term sc'
 quoteList a th [] = nil
